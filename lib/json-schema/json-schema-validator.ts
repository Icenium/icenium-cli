...
export class JsonSchemaValidator implements IJsonSchemaValidator {
	...
	public validatePropertyUsingBuildSchema(propertyName: string, propertyValue: string): void {
		...
		if(_.keys(invalidPlatforms).length === this.$mobileHelper.platformNames.length) {
			this.$errors.fail("The validation of provided %s property failed with following errors:\n", propertyName, _.values(invalidPlatforms).join("\n"));
		}
	...
	private getValidationErrors(data: IProjectData, opts?: {validationSchemaName?: string; usePredefinedErrors?: boolean}): IStringDictionary {
		...
		_.each(errors, (error: any) => {
			...
			var validationSchema = this.tryResolveValidationSchemaCore(undefined, opts.validationSchemaName);
			result[propertyName] = (opts.usePredefinedErrors === false ? undefined : JsonSchemaValidator.PREDEFINED_ERRORS[propertyName]) || validationSchema.properties[propertyName].errorMessage || errorMessage;
		});
