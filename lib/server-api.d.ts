...
declare module Server {
	...
	interface IAuthenticationServiceContract {
		...
		getAccountsFromTenant(): Promise<Server.Tenant[]>;
	...
	interface IAccountsIdentityStoreServiceContract {
		getIdentities(accountId: string): Promise<Server.CryptographicIdentityData[]>;
		generateSelfSignedIdentity(accountId: string, generationData: Server.IdentityGenerationData): Promise<Server.CryptographicIdentityData>;
		importIdentity(accountId: string, importType: Server.ImportType, password: string, stream: any): Promise<Server.CryptographicIdentityData[]>;
		removeIdentity(accountId: string, identityAlias: string): Promise<void>;
		getIdentity(accountId: string, identityAlias: string, password: string, $resultStream: any): Promise<void>;
		getCertificateRequests(accountId: string): Promise<Server.CertificateRequestData[]>;
		generateCertificationRequest(accountId: string, subjectNameValues: IDictionary<string>): Promise<Server.CertificateRequestData>;
		removeCertificateRequest(accountId: string, uniqueName: string): Promise<void>;
		getCertificateRequest(accountId: string, uniqueName: string, $resultStream: any): Promise<void>;
	}
	...
	interface IAccountsMobileprovisionsServiceContract {
		getProvisions(accountId: string): Promise<Server.ProvisionData[]>;
		importProvision(accountId: string, provision: any): Promise<Server.ProvisionData>;
		getProvision(accountId: string, identifier: string, $resultStream: any): Promise<void>;
		removeProvision(accountId: string, identifier: string): Promise<void>;
	}
	...
	interface ApplicationServiceData {
		Type: string;
		Placeholders: IDictionary<string>;
		Comparer: any;
		Count: number;
		Keys: string[];
	...
	interface NpmPackage {
		...
		Author: Server.NpmAuthor;
		License: string;
		Keywords: string[];
		LatestVersion: string;
	}
	interface INpmServiceContract {
		queryNpmSearch(count: number, query: string, start: number): Promise<Server.NpmSearchResult>;
	...
	interface PluginSpecs {
		IsDirty: boolean;
		Item: Server.PluginSpec;
		Keys: string[];
		Values: Server.PluginSpec[];
		Count: number;
		IsReadOnly: boolean;
	}
	...
	interface IServer {
		...
		accountsIdentityStore: Server.IAccountsIdentityStoreServiceContract;
		...
		accountsMobileprovisions: Server.IAccountsMobileprovisionsServiceContract;
		...
		appsProjectPlugins: Server.IAppsProjectPluginsServiceContract;
