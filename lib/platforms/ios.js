(function() {
	...
	function install(packageFile,packageName, provisionedDevices) {
		...
		log.trace("device id %s", deviceId);

		var isFoundDeviceIdInProvision = _.find(provisionedDevices, function(device){
			return device === deviceId;
		});

		if(!isFoundDeviceIdInProvision || isFoundDeviceIdInProvision === undefined) {
			helpers.abort("The device is not included in the provisioning profile.");
		}

		var InstallationProxyClient = require("./ios/ios-mobile-device/installation-proxy/installation-proxy-client"),
		    installationProxyClient = new InstallationProxyClient();
