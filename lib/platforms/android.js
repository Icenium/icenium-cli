(function() {
	...
	function requestDeviceById(deviceId) {
		var noDeviceErrorMesage = util.format("There isn't a device with %s", deviceId.index || deviceId.serialNumber);

		if (deviceId.index && deviceId.index <= 0) {
			throw new Error(noDeviceErrorMesage);
		}
	...
	function requestAllDevices() {
		var requestAllDevicesCommand = "adb devices";
	...
	function composeCommand(deviceSerialNumber, command) {
		var result = util.format("adb -s %s", deviceSerialNumber);
