...
(function() {
	...
	function requestDeviceByIndex(index) {
		var noDeviceErrorMessage = util.format("There isn't a device with index %s", index);
		if (index <= 0) {
			return noDeviceErrorMessage;
		}
		return requestAllDevices()
			.then(function(devices) {
	...
	function requestDeviceBySerialNumber(serialNumber) {
		var noDeviceErrorMessage = util.format("There isn't a device with serialNumber %s", serialNumber);
		return requestAllDevices()
			.then(function(devices) {
