(function() {
	...
	function hasDevices(platform) {
		var platformsServices = getPlatformServices(platform);
		return Q.fcall(hasDevicesOfPlatforms, _.toArray(platformsServices));
	}

	function hasDevicesOfPlatforms(platformsServicesArray) {
		if (platformsServicesArray.length > 0) {
			var platformsService = platformsServicesArray[0];
			return platformsService.hasDevices()
				.then(function(result) {
					if (result) {
						return true;
					} else {
						hasDevicesOfPlatforms(platformsServicesArray.slice(1));
					}
				});
		} else {
			return false;
