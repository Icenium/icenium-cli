...
export class MultipartUploadService implements IMultipartUploadService {
	...
	private uploadChunk(path: string, startingIndex: number, endIndex: number, content: ReadableStream, fileSize: number): IFuture<void> {
		...
		return this.$serviceProxy.call<void>('UploadChunk', 'PUT', ['api', 'upload', encodeURI(path.replace(/\\/g, '/'))].join('/') + '?' + querystring.stringify({ 'hash': chunkHash }), null, [{ name: 'content', value: content, contentType: 'application/octet-stream' }], null, headers);
	}
}
