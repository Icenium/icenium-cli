...
export class MultipartUploadService implements IMultipartUploadService {
	...
	public uploadFileByChunks(filePath: string, bucketKey: string): IFuture<void> {
		return (() => {
			...
			while(chunkStartByte < fileSize) {
				...
				// Fibers crash in case we start multiple uploads simultaneously, so wait for each future.
				this.uploadChunk(bucketKey, chunkStartByte, endByte, chunkStream, fileSize).wait();
	...
	private uploadChunk(path: string, startingIndex: number, endIndex: number, content: ReadableStream, fileSize: number): IFuture<void> {
		return (() => {
			...
			// TODO: Implement chunk hash. Currently the server validates only file hash, so it is not required at the moment.
			var chunkHash = "";
			// hack to override chunkUpload as in autogenerated code we cannot specify Content-Range header.
			return this.$serviceProxy.call<void>('UploadChunk', 'PUT', ['api', 'upload', encodeURI(path.replace(/\\/g, '/'))].join('/') + '?' + querystring.stringify({ 'hash': chunkHash }), null, [{ name: 'content', value: content, contentType: 'application/octet-stream' }], null, headers).wait();
		}).future<void>()();
	}
}
