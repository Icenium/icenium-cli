...
export abstract class PluginsServiceBase implements IPluginsService {
	...
	public async fetch(pluginIdentifier: string): Promise<string> {
		...
		let plugin = _.find(await this.getAvailablePlugins(), (pl: IPlugin) => pl.data.Identifier.toLowerCase() === pluginIdentifier || pl.data.Name.toLowerCase() === pluginIdentifier);
	...
	public abstract async getAvailablePlugins(pluginsCount?: number): Promise<IPlugin[]>;
