...
module Commands {
	export class FetchPluginCommandData implements commands.Commands.ICommandData {
	...
	export class FetchPluginCommandDataFactory implements commands.Commands.ICommandDataFactory {
		public fromCliArguments(args: string[]): FetchPluginCommandData {
			return new FetchPluginCommandData(args);
		}
	}
	global.$injector.register("fetchPluginCommandDataFactory", FetchPluginCommandDataFactory);

	export class FetchPluginCommand implements commands.Commands.ICommand<FetchPluginCommandData> {
		...
		public execute(data: FetchPluginCommandData): void {
			if (data.Keywords.length === 0) {
				this.log.error("You must specify local path, URL to a plugin repository, name or keywords of a plugin published to the Cordova Plugin Registry.");
	...
	global.$injector.register("fetch-plugin", FetchPluginCommand);
}