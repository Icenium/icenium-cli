...
export class UserStatusCommand implements ICommand {
	...
	public execute(args:string[]): IFuture<void> {
		return (() => {
			...
			if (user.tenant) {
				fields["License"] = util.format("Telerik %s Edition (%s)", user.tenant.name, user.tenant.license);
			...
			var maxPrefixLength = _.max(fieldNames, (name: string) => name.length).length;
