...
export class IdentityManager implements Server.IIdentityManager {
	...
	public async autoselectProvision(appIdentifier: string, provisionTypes: string[], deviceIdentifier?: string): Promise<IProvision> {
		...
		let validator: IAsyncValidator<any> = this.$injector.resolve(iosValidators.IOSDeploymentValidator,
		...
		_.each(provisions, (prov: IProvision) => {
			...
			if (validationResult.isSuccessful) {
				hasCompatibleCertificate = _.some(identities, (identity: ICryptographicIdentity) => validator.validateCertificate(identity, prov).isSuccessful);
...
export interface IIdentityInformationGatherer {
	gatherIdentityInformation(defaults: IIdentityInformation): Promise<IIdentityInformation>;
}

class IdentityInformationGatherer implements IIdentityInformationGatherer {
	constructor(private $selfSignedIdentityValidator: IValidator<ISelfSignedIdentityModel>,
		private $userDataStore: IUserDataStore,
		private $prompter: IPrompter,
		private $httpClient: Server.IHttpClient,
		private $logger: ILogger) { }

	public async gatherIdentityInformation(model: IIdentityInformation): Promise<IIdentityInformation> {
...
export class ListCertificatesCommand implements ICommand {
	...
	public async execute(args: string[]): Promise<void> {
		await this.$identityManager.listCertificates();
	}
...
export class RemoveCryptographicIdentity implements ICommand {
	...
	public allowedParameters: ICommandParameter[] = [this.$stringParameterBuilder.createMandatoryParameter("Specify certificate name or index.")];
...
export class ImportCryptographicIdentity implements ICommand {
	...
	public allowedParameters: ICommandParameter[] = [this.$stringParameterBuilder.createMandatoryParameter("No certificate file specified."),
	new commandParams.StringCommandParameter(this.$injector)];
...
class CreateCertificateSigningRequest implements ICommand {
	...
	public allowedParameters: ICommandParameter[] = [new commandParams.StringCommandParameter(this.$injector),
	new commandParams.StringCommandParameter(this.$injector),
	new commandParams.StringCommandParameter(this.$injector)];
...
class RemoveCertificateSigningRequestCommand implements ICommand {
	...
	public allowedParameters: ICommandParameter[] = [this.$stringParameterBuilder.createMandatoryParameter("Specify certificate signing request index to delete.")];
...
interface ICertificateDownloader {
	downloadCertificate(uniqueName: string): Promise<void>;
}
...
class ImportProvisionCommand implements ICommand {
	...
	public allowedParameters: ICommandParameter[] = [new FileNameCommandParameter(this.$errors, this.$fs)];
...
class RemoveProvisionCommand implements ICommand {
	...
	public allowedParameters: ICommandParameter[] = [new ProvisionIdCommandParameter(this.$identityManager)];
