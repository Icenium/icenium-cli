(function() {
	...
	function createFromTemplate() {
		...
		projectDir = path.join(projectDir, options.appname);

		if (options.appid === undefined) {
			options.appid = generateDefaultAppId(options.appname);
		...
		if (fs.existsSync(templateFileName)) {
			createProjectFile(projectDir, options.appname, {AppIdentifier: options.appid})
				.then(function() {
					return extractTemplate(templateFileName, projectDir);
				})
				.then(function() {
					console.log(util.format("%s has been successfully created.", options.appname));
				})
				.catch(function(error) {
