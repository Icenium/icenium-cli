(function() {
	...
	function updateProjectProperty(projectData, mode, property, newValue) {
		var propData = JSON.parse(fs.readFileSync(path.join(__dirname, "../resources/project-properties.json")))[property];

		if (!propData) {
			helpers.abort("Unrecognized property '%s'", property);
		}
		...
		if (range) {
			var validValues;
			if (_.isArray(range)) {
				validValues = helpers.toHash(range, _.identity, _.identity);
			} else {
				validValues = helpers.toHash(range,
					function(value, key) { return value.input || key; },
	...
	function printProjectProperty(property) {
		ensureProject();
		console.log(projectData[property] || "Unrecognized property");
	}
