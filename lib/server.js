...
(function() {
	...
	function setProjectProperties(solutionName, projectName, solutionSpace, properties, callback) {
		...
		parameters.headers["Content-Type"] = "application/vnd.icenium+json";
		parameters.setSolutionSpace(solutionSpace);
	...
	function getLiveSyncUrl(filesystemPath, liveSyncToken, callback) {
		if (false) {
		} else {
			var fullDownloadPath = util.format("http://%s/Mist/MobilePackage/manifest?packagePath=%s&token=%s",
				config.ICE_SERVER, querystring.escape(filesystemPath), querystring.escape(liveSyncToken));
			...
			httpRequest(parameters, function(result) {
				...
				var itmsUrl = "itms-services://?action=download-manifest&amp;url=" + querystring.escape(JSON.parse(result.body));
				log.debug("ITMS URL '%s'", itmsUrl);
				callback(null, itmsUrl);
			})
		}
	}
			...
			function() {
				getLiveSyncToken(solutionName, projectName, solutionSpace, function(err, liveSyncToken) {
					...
					httpRequest(parameters, function(result) {
						...
						var buildResults = body.ResultsByTarget.Build.Items.$values;
						buildResults = buildResults.map(function(buildResult) {
							var fullPath = buildResult.FullPath.replace(/\\/g, "/");
							var downloadUrl = solutionSpace
								? util.format("raw/%s/%s/%s/%s", solutionSpace, solutionName, projectName, fullPath)
								: util.format("raw/%s/%s/%s", solutionName, projectName, fullPath);
