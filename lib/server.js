...
(function() {
	...
	function getLiveSyncUrl(platform, filesystemPath, liveSyncToken, callback) {
		var isIosInstall = (platform.toLowerCase() === "ios");

		var fullDownloadPath = util.format("http://%s/Mist/MobilePackage/%s?packagePath=%s&token=%s",
			config.ICE_SERVER, isIosInstall ? "manifest" : "package",
			querystring.escape(filesystemPath), querystring.escape(liveSyncToken));
