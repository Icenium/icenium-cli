...
(function() {
	...
	function importProject(solutionName, projectName, solutionSpace, projectZip, callback) {
		...
		var parameters = {
			host: config.ICE_SERVER,
			path: util.format("/api/projects/importProject/%s/%s", solutionName, projectName),
			method: "POST",
			headers: {
				"Content-Type": "application/octet-stream",
				"Content-Length": fs.statSync(projectZip).size,
				"Cookie": ".ASPXAUTH=" + getCookie(),
				"X-Transfer-Mode": "Stream",
			},
			body: fs.createReadStream(projectZip)
		};

		if (solutionSpace) {
			parameters.headers["X-Icenium-SolutionSpace"] = solutionSpace;
		}

		httpRequest(parameters, function(result) {
			var response = result.response;
			log.info("Import HTTP status code: %s", response.statusCode);
