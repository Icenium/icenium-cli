add-editablelistform
==========

Usage | Synopsis
------|-------
General | `$ appbuilder add-editablelistform` [--answers <File Path>]

Inserts an editable form in an existing editable list. <% if(isHtml) { %>An interactive prompt guides you through the setup process.<% } %>
...
* `<File Path>` is the absolute or relative file path to a `JSON` file which contains configuration information about your project.<% if(isHtml) { %> The file must comply with the JSON specification described in detail [here](http://docs.telerik.com/platform/appbuilder/creating-your-project/screen-builder-automation#add-editablelistform).<% } %>

<% if(isHtml) { %>
### Prerequisites

* You must have at least one editable list configured with `$ appbuilder add-editablelist`
