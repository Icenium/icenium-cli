...
Usage | Synopsis
------|-------
General | `$ appbuilder add-editablelist` [--answers <File Path>]

Inserts an editable list in an existing application view. <% if(isHtml) { %>An interactive prompt guides you through the setup process.<% } %>
...
* `<File Path>` is the absolute or relative file path to a `JSON` file which contains configuration information about your project.<% if(isHtml) { %> The file must comply with the JSON specification described in detail [here](http://docs.telerik.com/platform/appbuilder/creating-your-project/screen-builder-automation#add-editablelist).<% } %>
